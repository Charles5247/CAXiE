...
const stats = [
  { label: 'Blog Posts', key: 'blog', color: 'text-blue-400', bg: 'bg-blue-400/10' },
  { label: 'Open Roles', key: 'jobs', color: 'text-green-400', bg: 'bg-green-400/10' },
  { label: 'Products', key: 'products', color: 'text-yellow-400', bg: 'bg-yellow-400/10' },
  { label: 'Case Studies', key: 'cases', color: 'text-brand-400', bg: 'bg-brand-600/10' },
];
        ...
        <div className="p-3 border-t border-white/10">
          <div className="flex items-center gap-2 px-3 py-2 mb-1">
            <div className="w-7 h-7 bg-brand-600/30 rounded-full flex items-center justify-center flex-shrink-0">
              <span className="text-brand-400 text-xs font-bold">{session?.email?.[0]?.toUpperCase() || 'A'}</span>
            </div>
            <div className="min-w-0">
              <p className="text-white text-xs font-medium truncate">{session?.email || 'Admin'}</p>
              <p className="text-gray-600 text-xs capitalize">{session?.role || 'administrator'}</p>
            </div>
          </div>
          <button
            onClick={onLogout}
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 text-sm">Manage your site content from here. Changes to Blog, Jobs, and Products are live immediately via the Supabase backend.</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {contentTypes.map((ct) => (
          ...
          >
            ...
            <h3 className="font-display font-semibold text-white mb-1 group-hover:text-brand-400 transition-colors">{ct.label}</h3>
      ...
      <div className="bg-brand-600/10 border border-brand-600/20 rounded-xl p-4">
        ...
        <p className="text-gray-400 text-xs leading-relaxed">
          Blog posts are read from and written to Supabase. Job listings and Products are managed via JSON data files (
          <code className="text-brand-300">data/jobs.json</code>, <code className="text-brand-300">data/products.json</code>) — deployable on Render with zero database overhead. For a fully dynamic CMS without code deploys, migrate Jobs and Products to Supabase tables using the schema in <code className="text-brand-300">lib/supabase-server.js</code>.
        </p>
      </div>
